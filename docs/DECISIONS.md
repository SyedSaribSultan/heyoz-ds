...
| A2 | Light `--sidebar-border` (96.27%) brighter than `--sidebar` (93.14%) | **Resolved twice.** First by rebuilding the sidebar off the shared ramp; then by deleting the namespace outright — the sidebar now uses `surface/secondary` + `border/secondary`, which are gated like every other surface/border pair. The inverted-border class of bug is now unreachable rather than merely fixed | `spec.mjs` (SIDEBAR deleted) | S |
...
> **A2 correction, and then its retirement.** The resolution above used to end
> "border always darker than its surface." That was true in light and false in dark,
> where `sidebar/border` was `neutral/120` on a `neutral/140` background —
> deliberately *lighter*, because the dark surface steps are compressed and a darker
> border disappears into them. The gate was a visibility floor, not a direction.
> **Both the token and the correction are now moot:** the sidebar namespace is gone,
> and `border/secondary` on `surface/secondary` is covered by the general visibility
> gates. Kept because the *lesson* survives the tokens — a visibility gate must not
> assume a direction.
>
...
| B12 | Sidebar was the only surface with component tokens, and its border was inverted | **Deleted, not rebuilt.** The first pass gave it 8 tokens off the shared ramp, which fixed the values and kept the anomaly the row is about — one surface with a private namespace. Four of the eight were byte-identical aliases of tier-2 roles (`content/primary`, `content/secondary`, `fill/selected`, `content/selected`); the other four made the sidebar the only region exempt from the elevation ladder. It uses the ladder now, and no surface has component tokens | `spec.mjs` (SIDEBAR deleted) | S |
...
|---|---|
| Does Bricolage hold at `heading xs` (18px)? If not, move `heading sm`/`xs` to Geist semibold — two lines in `spec.mjs` | showcase `/` § Typography |
| Is the `-variant` surface track earning its ~24 tokens? | after a week in Figma |
| Does `extrabold` earn its place on display? | showcase `/` § Typography |
| The light chart series are now noticeably darker (L\* 38–68 rather than 46–78), because 1.4.11 does not allow a pale series on a white page. Is that acceptable, or should charts get a tinted plot background so the series can go lighter again? | showcase `/` § Charts & gradients |
| ~~Six of 22 tier-3 tokens resolve to the same primitive pair as a tier-2 token~~ **Resolved.** Four of the six were the sidebar aliases and the namespace is deleted; `fill/selected` and `fill/brand-secondary` remain byte-identical and that pair is kept deliberately — the name is the semantics, and a selected row reaching for `fill/brand-secondary` would say the wrong thing about why it is tinted. So the answer to the architectural question is: the flat model is fine, and the duplicates that were noise are gone | resolved |
| **18 pairings sit below the 4.5:1 floor against a ground that is not a token, and `verify:composite` now measures all 408 of them.** Two families. **(a) Accent text on the top of the dark ladder** — on `surface/elevated` and `surface/overlay` (both `neutral/105`, `#393735`): `content/info` 3.89:1, `content/critical` 3.91, `content/warning` 3.93, `content/brand` and `content/link` 4.02, `content/success` 4.08. All six accents, twelve pairs. `content/tertiary` clears at 4.95 and `content/secondary` at 6.54, which is why B18's ladder work passed — the ceiling was set against neutral text and the accents were never checked at it. Those two surfaces are Dialog and Card/`overlay`, so it is reachable by any status text, link or accent word in a modal. **(b) Text on a translucent fill, composited over a real surface** — `fill/selected` is brand at 15% alpha, so its colour depends on what is beneath it. `content/selected` on it is 6.08:1 over the page and **3.55:1** over a dialog; `content/tertiary` is 4.36 there and **4.16 in LIGHT** over `surface/tertiary`. Six pairs. Ways out: move the six accents one ramp step in dark (all clear at their `-hover` step, 4.54–5.68), then handle (b) separately; or set `ENFORCING = true` in `verify-composite.ts` and let the build fail until they move. **Real blast radius** — `content/brand` in dark was what `verify:glow` modelled over the `/ai-ugc` hero gradient at a 5.01:1 tightest margin, and it is the accent word in that headline. That gate and that route are both gone (B24), so the constraint is now inert — but `/static-ads` has a gradient ground of its own under a display headline and an accent line, and rebuilding the glow model there would reimpose it. Not a `spec.mjs` change to make casually. Found while binding Field's error message; `field`, `listbox` and `dropzone` all route around it with the `-hover` step and say so. | `spec.mjs`; re-measure with `verify:composite` |
| **One of those 18 is not dark-only, and it kills the tempting one-line fix.** `content/tertiary` on `fill/selected` over `surface/tertiary` composites to `#E1C9C3` in **light** and measures 4.16:1 — a mid grey on a mid pink, neither near a boundary, simply nowhere to be far apart. So "move the accents one step in dark" does nothing for it: it is neither an accent nor dark. A selected row carrying a description needs `content/secondary` in both modes, which is what `listbox` already binds — arrived at from the dark case, before this one was known. | with (a) above, not before |
| Three of the eighteen entries in `verify-composite.ts`'s `KNOWN` list were wrong when first written, and the gate's own self-cleaning checks caught all three rather than a reviewer: two named a pairing that measured *nothing* (the token was missing from the fill's `on` list, so a known-bad entry read as coverage), and one named a pairing that *passes* at 4.54:1. A fourth failure — the light-mode one above — only appeared once the missing token was added. Worth noting as evidence for the shape: a ratchet that fails on "fixed but still listed" is what stops a known-issues list decaying into fiction. | resolved — kept here as the argument |
