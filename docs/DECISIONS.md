...
| **All six accent content tokens fail on the top two rungs of the dark surface ladder, and none of the twelve pairs is gated.** In dark, on `surface/elevated` and `surface/overlay` (both `neutral/105`, `#393735`): `content/brand` 4.02:1, `content/success` 4.08, `content/warning` 3.93, `content/critical` 3.89, `content/info` 3.89, `content/link` 4.02 — against a 4.5 floor. `content/tertiary` clears at 4.95 and `content/secondary` at 6.54, which is why B18's ladder work passed: the ceiling was set against neutral text and the accents were never checked at it. Those two surfaces are Dialog and Card/`overlay`, so this is reachable by any status text, link or accent word inside a modal. No shipped recipe pairs them yet, which is why all eight suites are green. Three ways out: move the six accents one ramp step in dark (they all clear at their `-hover` step, 4.54–5.68); add the twelve gates and let the build fail until they are moved; or rule accent text off floating surfaces, which Dialog makes untenable. **The first has real blast radius** — `content/brand` in dark is what `verify:glow` models over the `/ai-ugc` hero gradient at a 5.01:1 tightest margin, and it is the accent word in that headline. Not a `spec.mjs` change to make casually. Found while binding Field's error message, which routes around it with `content/critical-hover`; `listbox` and `dropzone` do the same. | `spec.mjs` + `verify-glow.ts`, together |
| A translucent fill measured over the page is not the same fill measured over a panel, and `verify:contrast` only ever does the first. `content/selected` on `fill/selected` is 6.08:1 over the page and **3.55:1** over `surface/overlay` — so the token pair named for each other is page-only, and the sweep reports the passing number. `listbox` uses `content/primary` instead and says so. This is the `verify:glow` blind spot in a second place: a composite is not a token. Worth a sweep that composites every alpha fill over every surface it can legally sit on? | `verify-contrast.ts` |
